...
//  CustomPlist
//
//  Created by Byoung_wook on 05/12/2019.
...
class ListViewController : UITableViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @objc func newAccount(_ sender: Any){
        ...
        let alert = UIAlertController(title: "계정을 입력하세요", message: nil, preferredStyle: .alert)
        
        alert.addTextField(){
            $0.placeholder = "예) liam@gmail.com"
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 1 && !(self.account.text?.isEmpty)! {
            ...
            alert.addAction(UIAlertAction(title: "OK", style: .default){
                (_) in let value = alert.textFields?[0].text
